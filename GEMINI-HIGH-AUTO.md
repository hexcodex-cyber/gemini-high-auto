...
- **Unattended Tasking**: This allows the AI to perform complex, multi-step operations (e.g., "Build a full-stack app from scratch") autonomously.
...
`gemini-high-auto` is designed to give you the speed of YOLO with a **Safety Net**. The `gemini-guard.ps1` script acts as a final gatekeeper, matching every command against `destructive_matchers.rules` *after* the AI has already "approved" it but *before* it hits your operating system.
